...
To manage your data effectively and safely, it is a good idea to know a little bit about database _normalization_: "Normalization is a process that database designers use to eliminate data redundancy, improve data integrity, and enhance the overall efficiency of the database." [^1]

The _normalization_ of data has nothing to do with the normal (Gaussian) distribution.

Data normalization it is not a case of either/or -- data can be normalized to different levels, depending on your needs.
...
* Variables to not contain arrays of atomic values
...
**Example 1**: More than one value is stored per cell. In other words, each cell consists of an array (or _a series of_) values, separated by a comma. 

**Example 2**: The data is not _atomic_, i.e. indivisible. The cells of columns 'test_a' and 'test_b' contain two different values (measurement and weekday).

**Example 3**: The data is atomic, and there is only one data point per cell. However, the data set contains repeating groups of similar variables ('a' and 'b').

**Example 4**: This data conforms to the First Normal Form (1NF) data structure.
:::
...
**Example 1**: The data does not contains a single-column primary key. Each observation is unique and identifiable by the (compound) primary key `subject_id` + `test_id`. 
...
See online for further details about data normalization - e.g. [this link](https://www.linkedin.com/pulse/understanding-normalization-databases-crucial-step-data-organization-dwnwc)
