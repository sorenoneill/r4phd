...
*  Explore `mpg`. Using `?mpg`, `view()`, `glimpse()`, `skimr()`, etc.
...
# Multiple Categorial
ggplot(mpg, aes(cty, displ, colour = class, shape = fl)) + 
  geom_point()
```
...
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy),
             color = "blue")  # color = "blue" must be placed outside the aes() function.
```
...
* Change something so that you can evaluate if race is equally distributed across the Installations
```{r}
soldiers %>% 
  ggplot(aes(y = Installation, fill = race))+
